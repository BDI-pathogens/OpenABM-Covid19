...
char *load_parameters_file_to_buffer( char *file_name )
{
    ...
    if (file_size >= INPUT_PARAMS_BUFFER_LEN) print_exit("Input buffer is too big");
...
char *load_household_demographics_file_to_buffer( parameters *params, char *file_name )
{
    ...
    if (file_size >= INPUT_HOUSEHOLD_BUFFER_LEN) print_exit("Input buffer is too big");
...
/*****************************************************************************************
*  Name:		update_param_from_buffer
*  Description: Update parameters from csv formatted buffer
******************************************************************************************/
void update_param_from_buffer( parameters *params, char *buffer )
{
    int i = 0;
	int check = 0;
    char *line, *token;
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, " %lf ,",  &(params->work_network_rewire));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%lf", &(params->sd_time_to_critical));
	...
	{
        token = strtok(NULL, ",");
		check = sscanf(token, "%lf", &(params->location_death_icu[i]));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%i", &(params->quarantine_length_traced_symptoms));
	if( check < 1){ print_exit("Failed to read parameter quarantine_length_traced_symptoms\n"); };

    token = strtok(NULL, ",");
	check = sscanf(token, "%i", &(params->quarantine_length_traced_positive));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%lf", &(params->quarantine_dropout_traced_symptoms));
	if( check < 1){ print_exit("Failed to read parameter quarantine_dropout_traced_symptoms\n"); };

    token = strtok(NULL, ",");
	check = sscanf(token, "%lf", &(params->quarantine_dropout_traced_positive));
	if( check < 1){ print_exit("Failed to read parameter quarantine_dropout_traced_positive\n"); };
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%lf", &(params->quarantine_compliance_traced_symptoms));
	if( check < 1){ print_exit("Failed to read parameter quarantine_compliance_traced_symptoms\n"); };

    token = strtok(NULL, ",");
	check = sscanf(token, "%lf", &(params->quarantine_compliance_traced_positive));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%i", &(params->test_release_on_negative));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, " %i ,", &(params->retrace_on_positive));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, " %i ,", &(params->quarantine_household_on_traced_positive));
	if( check < 1){ print_exit("Failed to read parameter quarantine_household_on_traced_positive\n"); };

    token = strtok(NULL, ",");
	check = sscanf(token, " %i ,", &(params->quarantine_household_on_traced_symptoms));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%i",   &(params->test_sensitive_period));
	if( check < 1){ print_exit("Failed to read parameter test_sensitive_period\n"); };

    token = strtok(NULL, ",");
	check = sscanf(token, "%lf",   &(params->test_sensitivity));
	if( check < 1){ print_exit("Failed to read parameter test_sensitivity\n"); };

    token = strtok(NULL, ",");
	check = sscanf(token, "%lf",   &(params->test_specificity));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%i",   &(params->test_order_wait_priority));
    ...
    token = strtok(NULL, ",");
	check = sscanf(token, "%i",   &(params->test_result_wait_priority));
	if( check < 1){ print_exit("Failed to read parameter test_result_wait_priority\n"); };

	for( i = 0; i < N_AGE_GROUPS; i++ )
    {
        token = strtok(NULL, ",");
	    check = sscanf(token, "%i",   &(params->priority_test_contacts[i]));
    ...
    {
        token = strtok(NULL, ",");
		check = sscanf(token, " %lf ,", &(params->lockdown_occupation_multiplier[i]));
...
void read_param_file( parameters *params)
{
    ...
    update_param_from_buffer(params, buffer);
    free(buffer);
}
...
void read_household_demographics_file( parameters *params)
{
    ...
    update_household_demographics_param_from_buffer(params, buffer);
    free(buffer);
}
