...
def lockdown_then_unlock_no_corona(data_path: str = 'data'):
    ...
    econ = Economics(SupplyDemandGdpModel(), CorporateBankruptcyModel(), PersonalBankruptcyModel())
    econ.load(reader)
    max_utilisations = {
        key: 1.0 for key in itertools.product(Region, Sector, Age)}
    min_utilisations = {
        key: 0.0 for key in itertools.product(Region, Sector, Age)}
    length = 100
    ...
    df = pd.DataFrame([econ.results.fraction_gdp_by_sector(i) for i in range(1, length)],
                      index=range(1, length)).T.sort_index().T.cumsum(axis=1)
    ...
    # Plot 2
    df = pd.DataFrame([econ.results.corporate_solvencies[i] for i in econ.results.corporate_solvencies])
    df.plot(figsize=(20, 10))

    # Plot 3
    pd.DataFrame([
        {r: econ.results.personal_bankruptcy[i][r].personal_bankruptcy for r in Region}
        for i in econ.results.personal_bankruptcy
    ]).plot(figsize=(20, 10))

    # Plot 4
    pd.DataFrame([
        {r: econ.results.personal_bankruptcy[i][r].corporate_bankruptcy for r in Region}
        for i in econ.results.personal_bankruptcy
    ]).plot(figsize=(20, 10))
