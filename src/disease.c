...
)
{
	...
	individual *infector;
	float hospital_location_modifier;
	...
	for( day = model->time-1; day >= max( 0, model->time - MAX_INFECTIOUS_PERIOD ); day-- )
	{
		...
		for( idx = 0; idx < n_infected; idx++ )
		{
			...
			if( n_interaction > 0 )
			{
                ...
                switch( infector->hospital_location )
                {
                    case WAITING:       hospital_location_modifier = params->waiting_infectivity_modifier; break;
                    case GENERAL:       hospital_location_modifier = params->general_infectivity_modifier; break;
                    case ICU:           hospital_location_modifier = params->icu_infectivity_modifier; break;
                    default: 			hospital_location_modifier = 1.0; // Not in hospital, rates unaffected.
                }
				...
				if(model->time - 1 - time_infected( infector ) >= MAX_INFECTIOUS_PERIOD)
					...
					hazard_rate = hazard_rate * hospital_location_modifier;
				}
