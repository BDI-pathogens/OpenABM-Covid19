...
class TestClass(object):
    params = {
        ...
        "test_monoton_asymptomatic_infectious_factor": [
            dict(
                end_time=250, asymptomatic_infectious_factor=[0, 0.25, 0.5, 0.5, 1, 0.1]
            )
        ],
        ...
        "test_ratio_presymptomatic_symptomatic": [
            dict(n_total=10000, n_seed_infection=1, end_time=100),
            dict(n_total=100000, n_seed_infection=10, end_time=1),
            dict(n_total=1000000, n_seed_infection=100, end_time=1),
            dict(n_total=10000000, n_seed_infection=1000, end_time=1),
            dict(n_total=100000, n_seed_infection=10000, end_time=1),
        ],
    }
    """
    ...
    def test_transmission_pairs(
        ...
        params.set_param(
            "hospitalised_daily_interactions", hospitalised_daily_interactions
        )
        ...
        df_trans = pd.read_csv(
            constant.TEST_TRANSMISSION_FILE, comment="#", sep=",", skipinitialspace=True
        )
        # check to see that the number of entries in the transmission file is that in the time-series
        np.testing.assert_equal(
            len(df_trans),
            df_output.loc[:, "total_infected"].max(),
            "length of transmission file is not the number of infected in the time-series",
        )
        ...
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"]
                    == constant.EVENT_TYPES.PRESYMPTOMATIC.value
                ]
            )
            > 0,
            True,
            "no transmission from presymptomatic people",
        )
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"]
                    == constant.EVENT_TYPES.PRESYMPTOMATIC_MILD.value
                ]
            )
            > 0,
            True,
            "no transmission from presymptomatic (mild) people",
        )
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"]
                    == constant.EVENT_TYPES.SYMPTOMATIC.value
                ]
            )
            > 0,
            True,
            "no transmission from symptomatic people",
        )
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"]
                    == constant.EVENT_TYPES.SYMPTOMATIC_MILD.value
                ]
            )
            > 0,
            True,
            "no transmission from symptomatic (mild) people",
        )
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"]
                    == constant.EVENT_TYPES.ASYMPTOMATIC.value
                ]
            )
            > 0,
            True,
            "no transmission from asymptomatic people",
        )
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"]
                    == constant.EVENT_TYPES.HOSPITALISED.value
                ]
            )
            > 0,
            True,
            "no transmission from hospitalised people",
        )
        np.testing.assert_equal(
            len(
                df_trans[
                    df_trans["infector_status"] == constant.EVENT_TYPES.CRITICAL.value
                ]
            )
            > 0,
            True,
            "no transmission from critical people",
        )

        # check the only people who were infected by someone after 0 time are the seed infections
        np.testing.assert_equal(
            min(df_trans["infector_infected_time"]),
            0,
            "the minimum infected time at transmission must be 0 (the seed infection",
        )
        np.testing.assert_equal(
            len(df_trans[df_trans["infector_infected_time"] == 0]),
            int(params.get_param("n_seed_infection")),
            "only the seed infection are infected by someone after 0 days",
        )

        # check that some people can get infected after one time step
        np.testing.assert_equal(
            len(df_trans[df_trans["infector_infected_time"] == 1]) > 0,
            True,
            "nobody is infected by someone who is infected by for one unit of time",
        )

        # check the maximum time people are stil infections
        max_sd = 7
        max_time = float(params.get_param("mean_infectious_period")) + max_sd * float(
            params.get_param("sd_infectious_period")
        )
        np.testing.assert_equal(
            max(df_trans["infector_infected_time"]) < max_time,
            True,
            "someone is infectious at a time greater than mean + 7 * std. dev. of the infectious curve ",
        )

        # check that some people are infected across all networks
        np.testing.assert_equal(
            sum(df_trans["infector_network"] == constant.HOUSEHOLD) > 0,
            True,
            "no transmission on the household network",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_network"] == constant.WORK) > 0,
            True,
            "no transmission on the work network",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_network"] == constant.RANDOM) > 0,
            True,
            "no transmission on the random network",
        )

        # check hospitalised people are not transmitting on the work and household networks
        np.testing.assert_equal(
            sum(
                (df_trans["infector_network"] == constant.HOUSEHOLD)
                & (df_trans["infector_status"] == constant.EVENT_TYPES.HOSPITALISED)
            ),
            0,
            "hospitalised people transmitting on the household network",
        )
        np.testing.assert_equal(
            sum(
                (df_trans["infector_network"] == constant.WORK)
                & (df_trans["infector_status"] == constant.EVENT_TYPES.HOSPITALISED)
            ),
            0,
            "hospitalised people transmitting on the work network",
        )
    ...
    def test_exponential_growth_homogeneous_random(
        ...
        params = ParameterSet(constant.TEST_DATA_FILE, line_number=1)
        params = utils.set_homogeneous_random_network_only(
            params, n_connections, end_time
        )
        params.set_param("infectious_rate", infectious_rate)
        params.set_param("mean_infectious_period", mean_infectious_period)
        params.set_param("sd_infectious_period", sd_infectious_period)
        ...
        params.set_param(
            "mean_time_to_recover", mean_infectious_period + 2 * sd_infectious_period
        )
        params.set_param(
        ...
        slope = (log(ts_2["total_infected"]) - log(ts_1["total_infected"])) / (
            ts_2["time"] - ts_1["time"]
        )
    ...
    def test_monoton_relative_transmission(
        self, end_time, transmission_NETWORK, relative_transmission_values
    ):
        """
        ...
        """
        relative_transmissions = [
            "relative_transmission_household",
            "relative_transmission_workplace",
        ...
        df_trans_current = pd.read_csv(
            constant.TEST_TRANSMISSION_FILE, comment="#", sep=",", skipinitialspace=True
        )

        # calculating the first ratio
        len_household = len(
            df_trans_current[df_trans_current["infector_network"] == constant.HOUSEHOLD]
        )
        len_work = len(
            df_trans_current[df_trans_current["infector_network"] == constant.WORK]
        )
        len_random = len(
            df_trans_current[df_trans_current["infector_network"] == constant.RANDOM]
        )
        lengths = [int(len_household), int(len_work), int(len_random)]
        all_trans_current = sum(lengths)
        ratio_current = float(
            df_trans_current[
                df_trans_current["infector_network"] == transmission_NETWORK
            ].shape[0]
        ) / float(all_trans_current)
        ...
        for relative_transmission_value in relative_transmission_values[1:]:
            ...
            df_trans = pd.read_csv(
                constant.TEST_TRANSMISSION_FILE,
                comment="#",
                sep=",",
            ...
            all_trans = (
                len(df_trans[df_trans["infector_network"] == constant.HOUSEHOLD])
                + len(df_trans[df_trans["infector_network"] == constant.WORK])
                + len(df_trans[df_trans["infector_network"] == constant.RANDOM])
            )
            ratio_new = float(
                df_trans[df_trans["infector_network"] == transmission_NETWORK].shape[0]
            ) / float(all_trans)
    ...
    def test_monoton_fraction_asymptomatic(
        ...
        for idx in range(1, len(fraction_asymptomatic_0_9)):
            params.set_param(
                "fraction_asymptomatic_0_9", fraction_asymptomatic_0_9[idx]
            )
            params.set_param(
                "fraction_asymptomatic_10_19", fraction_asymptomatic_10_19[idx]
            )
            params.set_param(
                "fraction_asymptomatic_20_29", fraction_asymptomatic_20_29[idx]
            )
            params.set_param(
                "fraction_asymptomatic_30_39", fraction_asymptomatic_30_39[idx]
            )
            params.set_param(
                "fraction_asymptomatic_40_49", fraction_asymptomatic_40_49[idx]
            )
            params.set_param(
                "fraction_asymptomatic_50_59", fraction_asymptomatic_50_59[idx]
            )
            params.set_param(
                "fraction_asymptomatic_60_69", fraction_asymptomatic_60_69[idx]
            )
            params.set_param(
                "fraction_asymptomatic_70_79", fraction_asymptomatic_70_79[idx]
            )
            ...
            fraction_asymptomatic_new = (
                fraction_asymptomatic_0_9[idx]
            ...
            if fraction_asymptomatic_new > fraction_asymptomatic_current:
                np.testing.assert_equal(
                    total_infected_new < total_infected_current, True
                )
                ...
                np.testing.assert_allclose(
                    total_infected_new, total_infected_current, atol=0.01
                )
    ...
    def test_monoton_asymptomatic_infectious_factor(
        self, end_time, asymptomatic_infectious_factor
    ):
        """
        ...
        params.set_param("end_time", end_time)
        params.set_param(
            "asymptomatic_infectious_factor", asymptomatic_infectious_factor[0]
        )
        params.write_params(constant.TEST_DATA_FILE)
        ...
        for idx in range(1, len(asymptomatic_infectious_factor)):
            params.set_param(
                "asymptomatic_infectious_factor", asymptomatic_infectious_factor[idx]
            )
            params.write_params(constant.TEST_DATA_FILE)
            ...
            # check the total infections
            if (
                asymptomatic_infectious_factor_new
                > asymptomatic_infectious_factor_current
            ):
                np.testing.assert_equal(
                    total_infected_new > total_infected_current, True
                )
    ...
    def test_monoton_relative_susceptibility(
        ...
        params.set_param(
            "relative_susceptibility_10_19", relative_susceptibility_10_19[0]
        )
        params.set_param(
            "relative_susceptibility_20_29", relative_susceptibility_20_29[0]
        )
        params.set_param(
            "relative_susceptibility_30_39", relative_susceptibility_30_39[0]
        )
        params.set_param(
            "relative_susceptibility_40_49", relative_susceptibility_40_49[0]
        )
        params.set_param(
            "relative_susceptibility_50_59", relative_susceptibility_50_59[0]
        )
        params.set_param(
            "relative_susceptibility_60_69", relative_susceptibility_60_69[0]
        )
        params.set_param(
            "relative_susceptibility_70_79", relative_susceptibility_70_79[0]
        )
        ...
        infected_current = (
            df_trans_current["age_group"].value_counts().sort_index(axis=0)
        )
        inf_cur_zeros = pd.concat(
            [inf_cur_zeros, infected_current], ignore_index=True, axis=1
        )
        infected_current = inf_cur_zeros[0] + inf_cur_zeros.fillna(0)[1]

        # get the relative_susceptibility for all age groups
        relative_susceptibility_current = [
            relative_susceptibility_0_9[0],
        ...
        for idx in range(1, len(relative_susceptibility_0_9)):
            params.set_param(
                "relative_susceptibility_0_9", relative_susceptibility_0_9[idx]
            )
            params.set_param(
                "relative_susceptibility_10_19", relative_susceptibility_10_19[idx]
            )
            params.set_param(
                "relative_susceptibility_20_29", relative_susceptibility_20_29[idx]
            )
            params.set_param(
                "relative_susceptibility_30_39", relative_susceptibility_30_39[idx]
            )
            params.set_param(
                "relative_susceptibility_40_49", relative_susceptibility_40_49[idx]
            )
            params.set_param(
                "relative_susceptibility_50_59", relative_susceptibility_50_59[idx]
            )
            params.set_param(
                "relative_susceptibility_60_69", relative_susceptibility_60_69[idx]
            )
            params.set_param(
                "relative_susceptibility_70_79", relative_susceptibility_70_79[idx]
            )
            params.set_param(
                "relative_susceptibility_80", relative_susceptibility_80[idx]
            )
            params.write_params(constant.TEST_DATA_FILE)
            ...
            relative_susceptibility_new = [
                relative_susceptibility_0_9[idx],
            ...
            inf_new_zeros = pd.concat(
                [inf_new_zeros, infected_new], ignore_index=True, axis=1
            )
            ...
            nonmatch_pairs = np.array(
                [
                    [int(i), curr, newr]
                    for i, (curr, newr) in enumerate(
                        zip(
    ...
    def test_ratio_presymptomatic_symptomatic(
        self, n_total, n_seed_infection, end_time
    ):
        """
        ...
        N_severe_pre = len(
            df_indiv[
                (df_indiv["time_infected"] > 0)
                & (df_indiv["time_presypmtomatic_severe"] > 0)
            ]
        )
        N_mild_pre = len(
            df_indiv[
                (df_indiv["time_infected"] > 0)
                & (df_indiv["time_presypmtomatic_mild"] > 0)
            ]
        )
        ...
        N_mild_sym = len(
            df_indiv[
                (df_indiv["time_infected"] > 0)
                & (df_indiv["time_symptomatic_mild"] > 0)
            ]
        )
        N_pre = N_severe_pre + N_mild_pre
        N_sym = N_severe_sym + N_mild_sym

        np.testing.assert_allclose(N_pre, N_sym, atol=tolerance)
