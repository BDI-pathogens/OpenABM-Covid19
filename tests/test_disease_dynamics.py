...
class TestClass(object):
    ...
    def test_disease_transition_times(self, test_params):
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_time_to_symptoms"],
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_time_to_hospital"],
            atol=std_error_limit * sd / sqrt(N),
        )
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_time_to_critical"],
            atol=std_error_limit * sd / sqrt(N),
        )
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_time_to_recover"],
            atol=std_error_limit * sd / sqrt(N),
        )
        np.testing.assert_allclose(
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_time_hospitalised_recovery"],
            atol=std_error_limit * sd / sqrt(N),
        )
        np.testing.assert_allclose(
            sd,
            test_params["sd_time_hospitalised_recovery"],
            atol=std_error_limit * sd / sqrt(N),
        )

        # time in ICU
        df_indiv["t_c_r"] = (
            df_indiv["time_hospitalised_recovering"] - df_indiv["time_critical"]
        )
        mean = df_indiv[
            (df_indiv["time_hospitalised_recovering"] > 0)
            & (df_indiv["time_critical"] > 0)
        ]["t_c_r"].mean()
        sd = df_indiv[
            (df_indiv["time_hospitalised_recovering"] > 0)
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_time_critical_survive"],
            atol=std_error_limit * sd / sqrt(N),
        )
        np.testing.assert_allclose(
            sd,
            test_params["sd_time_critical_survive"],
            atol=std_error_limit * sd / sqrt(N),
        )

        # time from ICU to death
        df_indiv["t_c_d"] = df_indiv["time_death"] - df_indiv["time_critical"]
        mean = df_indiv[(df_indiv["time_death"] > 0) & (df_indiv["time_critical"] > 0)][
            "t_c_d"
        ].mean()
        sd = df_indiv[(df_indiv["time_death"] > 0) & (df_indiv["time_critical"] > 0)][
            "t_c_d"
        ].std()
        N = len(
            df_indiv[(df_indiv["time_death"] > 0) & (df_indiv["time_critical"] > 0)]
        )
        np.testing.assert_allclose(
        ...
        np.testing.assert_allclose(
            mean,
            test_params["mean_asymptomatic_to_recovery"],
            atol=std_error_limit * sd / sqrt(N),
        )
        np.testing.assert_allclose(
            sd,
            test_params["sd_asymptomatic_to_recovery"],
            atol=std_error_limit * sd / sqrt(N),
        )
    ...
    def test_disease_outcome_proportions(self, test_params):
        ...
        for idx in range(constant.N_AGE_GROUPS):
            ...
            N_symp = len(
                df_indiv[
                    (df_indiv["time_presypmtomatic"] > 0)
                    & (df_indiv["time_presypmtomatic_severe"] > 0)
                    & (df_indiv["age_group"] == constant.AGES[idx])
                ]
            )
            ...
            np.testing.assert_allclose(
                mean_mild, mild_fraction[idx], atol=std_error_limit * sd_mild / sqrt(N),
            )
        ...
        np.testing.assert_allclose(
            mean_mild, mild_fraction_weighted, atol=std_error_limit * sd_mild / sqrt(N),
        )
        ...
        for idx in range(constant.N_AGE_GROUPS):
            ...
            sd = sqrt(
                max(
                    mean * (1 - mean),
                    hospitalised_fraction[idx] * (1 - hospitalised_fraction[idx]),
                )
            )
            np.testing.assert_allclose(
                mean,
