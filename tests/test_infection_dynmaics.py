...
class TestClass(object):
    ...
    def test_transmission_pairs(
        self, n_total, end_time, infectious_rate, hospitalised_daily_interactions
    ):
        ...
        df_trans = pd.read_csv(
            TEST_TRANSMISSION_FILE, comment="#", sep=",", skipinitialspace=True
        )
        ...
        np.testing.assert_equal(
            sum(df_trans["infector_status"] == PRESYMPTOMATIC) > 0,
            True,
            "no transmission from presymptomatic people",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_status"] == SYMPTOMATIC) > 0,
            True,
            "no transmission from symptomatic people",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_status"] == ASYMPTOMATIC) > 0,
            True,
            "no transmission from asymptomatic people",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_status"] == HOSPITALISED) > 0,
            True,
            "no transmission from hospitalised people",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_status"] == CRITICAL) > 0,
            True,
        ...
        np.testing.assert_equal(
            sum(df_trans["infector_network"] == HOUSEHOLD) > 0,
            True,
            "no transmission on the household network",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_network"] == WORK) > 0,
            True,
            "no transmission on the work network",
        )
        np.testing.assert_equal(
            sum(df_trans["infector_network"] == RANDOM) > 0,
            True,
        ...
        np.testing.assert_equal(
            sum(
                (df_trans["infector_network"] == HOUSEHOLD)
                & (df_trans["infector_status"] == HOSPITALISED)
            ),
            0,
        ...
        np.testing.assert_equal(
            sum(
                (df_trans["infector_network"] == WORK)
                & (df_trans["infector_status"] == HOSPITALISED)
            ),
            0,
