...
class TestClass(object):
    ...
    def test_quarantine_interactions(
        self, 
        test_params
    ):   
        """
        ...
        df_test = pd.merge( df_quar, df_int[ df_int["type"] == WORK ], on = "ID", how = "inner" )
        np.testing.assert_equal( len( df_test ), 0, "quarantined individual with work contacts" )

        # check to see there are are household connections 
        df_test = pd.merge( df_quar, df_int[ df_int["type"] == HOUSEHOLD ], on = "ID", how = "inner" )
        np.testing.assert_equal( len( df_test ) > 0, True, "quarantined individuals have no household connections" )

        # check to whether the number of random connections are as specified
        df_test = pd.merge( df_quar, df_int[ df_int["type"] == RANDOM ], on = "ID", how = "left" )
