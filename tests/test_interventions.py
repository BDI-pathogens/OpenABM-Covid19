...
class TestClass(object):
    ...
    def test_quarantine_household_on_symptoms(self, test_params):
        ...
        df_trace = pd.read_csv(
            constant.TEST_TRACE_FILE, comment="#", sep=",", skipinitialspace=True
        )
        ...
        index_traced = df_trace[
            (df_trace["time"] == end_time) & (df_trace["days_since_contact"] == 0)
        ]
        index_traced = (
            index_traced.groupby(["index_ID", "traced_ID"])
            .size()
        ...
        index_inter = (
            index_inter.groupby(["index_ID", "traced_ID", "household"])
            .size()
            .reset_index(name="N")
        )
        ...
        t = pd.merge(
            index_traced, index_inter, on=["index_ID", "traced_ID"], how="outer"
        )
        ...
        np.testing.assert_equal(
            n_no_trace, 0, "failed to trace someone in the household"
        )
    ...
    def test_lockdown_transmission_rates(self, test_params):
        ...
        df_without = (
            df_without[df_without["time_infected"] == end_time]
            .groupby(["infector_network"])
            .size()
            .reset_index(name="N")
        )
        ...
        df_with = (
            df_with[df_with["time_infected"] == end_time]
            .groupby(["infector_network"])
            .size()
            .reset_index(name="N")
        )
        ...
        expect_household = (
            df_without.loc[constant.HOUSEHOLD, ["N"]]
            * test_params["lockdown_house_interaction_multiplier"]
        )
        np.testing.assert_allclose(
            df_with.loc[constant.HOUSEHOLD, ["N"]],
            expect_household,
            atol=sqrt(expect_household) * sd_diff,
            err_msg="lockdown not changing household transmission as expected",
        )

        expect_work = (
            df_without.loc[constant.WORK, ["N"]]
            * test_params["lockdown_work_network_multiplier"]
        )
        np.testing.assert_allclose(
            df_with.loc[constant.WORK, ["N"]],
            expect_work,
            atol=sqrt(expect_work) * sd_diff,
            err_msg="lockdown not changing work transmission as expected",
        )

        expect_random = (
            df_without.loc[constant.RANDOM, ["N"]]
            * test_params["lockdown_random_network_multiplier"]
        )
        np.testing.assert_allclose(
            df_with.loc[constant.RANDOM, ["N"]],
            expect_random,
            atol=sqrt(expect_random) * sd_diff,
    ...
    def test_app_users_fraction(self, test_params):
        ...
        app_users = (
            df_indiv[df_indiv["app_user"] == 1]
            .groupby(["age_group"])
            .size()
            .reset_index(name="app_users")
        )
        ...
        for age in constant.AGES:
            if test_params[app_params[age]] == 0:
                ...
                np.testing.assert_equal(
                    len(users), 0, "nobody should have a phone in this age group"
                )
            else:
                ...
                np.testing.assert_allclose(
                    users / n,
                    test_params[app_params[age]],
                    atol=0.01,
                    err_msg="wrong fraction of users have app in age group",
                )
        ...
        index_traced = index_traced[
            index_traced["index_ID"] != index_traced["traced_ID"]
        ]
