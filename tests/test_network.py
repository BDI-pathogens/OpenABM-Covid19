...
class TestClass(object):
    ...
    def test_random_network(
        ...
        # note when counting connections we count each end
        ageTypeMap = pd.DataFrame(
            data={"age_group": constant.AGES, "age_type": constant.AGE_TYPES}
        )
        ...
        mean = (
            df_int[df_int["age_type"] == constant.ELDERLY].loc[:, "connections"].mean()
        )
    ...
    def test_work_network(
        ...
        mean = (
            df_n_int[df_n_int["work_network"] == constant.NETWORK_20_69]
            .loc[:, "connections"]
            .mean()
        )
