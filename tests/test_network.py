...
class TestClass(object):
    ...
    def test_household_network(self, n_total):
        ...
        expectedConnections = pd.DataFrame(
            data={"size": [1, 2, 3, 4, 5, 6], "expected": [0, 2, 6, 12, 20, 30]}
        )
        ...
        np.testing.assert_array_equal(
            df_int.loc[:, "house_no"], df_int.loc[:, "house_no_2"]
        )
        ...
        N_nocon_single = (
            df_house[df_house["size"] == 1].loc[:, ["connections"]].isnull().sum().sum()
        )
        ...
        print(df_house.head())
        np.testing.assert_array_equal(
            df_house.loc[:, "connections"], df_house.loc[:, "expected"]
        )
    ...
    def test_random_network(
        ...
        params = utils.turn_off_interventions(params, 1)
        params.set_param(
            "mean_random_interactions_child", mean_random_interactions_child
        )
        params.set_param("sd_random_interactions_child", sd_random_interactions_child)
        params.set_param(
            "mean_random_interactions_adult", mean_random_interactions_adult
        )
        params.set_param("sd_random_interactions_adult", sd_random_interactions_adult)
        params.set_param(
            "mean_random_interactions_elderly", mean_random_interactions_elderly
        )
        params.set_param(
            "sd_random_interactions_elderly", sd_random_interactions_elderly
        )
        params.set_param("n_total", n_total)
        params.write_params(TEST_DATA_FILE)
        ...
        np.testing.assert_allclose(
            mean, mean_random_interactions_elderly, rtol=tolerance
        )
        if mean_random_interactions_elderly > 0:
            np.testing.assert_allclose(
                sd, sd_random_interactions_elderly, rtol=tolerance
            )
    ...
    def test_work_network(
        ...
        params.set_param(
            "mean_work_interactions_elderly", mean_work_interactions_elderly
        )
        ...
        for age in AGES:
            if paramByNetworkType[NETWORK_TYPES[AGE_TYPES[age]]] > 0:
                ...
                np.testing.assert_equal(
                    n > 0,
                    True,
                    "there are no work connections for age_group " + str(age),
                )

        # check the correct people are on each network
        n = sum(
            (df_int["age_group"] == AGE_0_9)
            & (df_int["age_group_2"] != AGE_0_9)
            & (df_int["age_type_2"] != ADULT)
        )
        np.testing.assert_equal(n, 0, "only 0_9 and adults on the 0_9 network")
        n = sum(
            (df_int["age_group"] == AGE_10_19)
            & (df_int["age_group_2"] != AGE_10_19)
            & (df_int["age_type_2"] != ADULT)
        )
        np.testing.assert_equal(n, 0, "only 10_19 and adults on the 10_19 network")
        n = sum(
            (df_int["age_group"] == AGE_70_79)
            & (df_int["age_group_2"] != AGE_70_79)
            & (df_int["age_type_2"] != ADULT)
        )
        np.testing.assert_equal(n, 0, "only 70_79 and adults on the 70_79 network")
        n = sum(
            (df_int["age_group"] == AGE_80)
            & (df_int["age_group_2"] != AGE_80)
            & (df_int["age_type_2"] != ADULT)
        )
        ...
        for network in [NETWORK_0_9, NETWORK_10_19]:
            mean = (
                df_n_int[df_n_int["work_network"] == network]
                .loc[:, "connections"]
                .mean()
            )
            np.testing.assert_allclose(
                mean, mean_work_interactions_child, rtol=tolerance
            )

        mean = (
            df_n_int[df_n_int["work_network"] == NETWORK_20_69]
            .loc[:, "connections"]
            .mean()
        )
        ...
        for network in [NETWORK_70_79, NETWORK_80]:
            ...
            np.testing.assert_allclose(
                mean, mean_work_interactions_elderly, rtol=tolerance
            )
