...
class TestClass(object):
    ...
    def test_demographic_proportions(
        ...
        for idx in range(constant.N_AGE_GROUPS):
            ...
            np.testing.assert_allclose(
                N, population_fraction[idx], atol=N_tot * error_tolerance
            )
    ...
    def test_household_size(
        ...
        household_size_counts_weighted *= float(n_total) / sum(
            household_size_counts_weighted
        )

        # Run the simulation.
        file_output = open(constant.TEST_OUTPUT_FILE, "w")
        completed_run = subprocess.run(
            [constant.command], stdout=file_output, stderr=file_output, shell=True
        )
        ...
        df_house = df_house.groupby(["size"]).size().reset_index(name="house_count")
        df_house["people_count"] = df_house["size"] * df_house["house_count"]
        df_house["people_count_expected"] = household_size_counts_weighted

        # Test!
        np.testing.assert_allclose(
            df_house["people_count"], df_house["people_count_expected"], rtol=0.02
        )
